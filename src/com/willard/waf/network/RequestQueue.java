...
public class RequestQueue {
	...
	private NetworkDispatcher[] mDispatchers;// 分发器数组，可以设置多个分发器，即多个线程
	...
	public <T> Request<T> add(Request<T> request) {
		...
		// current requests.
		request.setRequestQueue(this);// 设置这个请求属于的队列 一个请求只属于一个队列，一个队列可以有多个请求
		synchronized (mCurrentRequests) {
		...
		if (request.mPreListener != null)
			request.mPreListener.onPreExecute();// 此时仍在主线程，而非子线程，所以无需使用mDelivery来处理，直接调用request
		...
		synchronized (mWaitingRequests) {
			...
			if (mWaitingRequests.containsKey(cacheKey)) {// 是否有相同请求正在被处理
				...
				// now a request in
				// flight.
				mWaitingRequests.put(cacheKey, null);// 创建新的等待当前请求的空队列
				mCacheQueue.add(request);// 添加当前请求到缓存请求队列mCacheQueue中
			}
			return request;
		}
	}
