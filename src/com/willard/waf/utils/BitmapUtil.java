...
public class BitmapUtil {
	private static final String clazzName = BitmapUtil.class.getSimpleName();
	...
	public static Bitmap getBitmap(String url) {
		...
		try {
			...
			LogUtil.d(clazzName, "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	public static Bitmap getSquareBitmap(Bitmap bitmap, int reqSize) {
		...
		try {
			...
			resizeBmp = Bitmap.createBitmap(bitmap, 0, 0, width, height,
					matrix, false);
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			LogUtil.e(clazzName, "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			LogUtil.e(clazzName, "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			LogUtil.e(clazzName, "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
	...
	public static byte[] view2Bytes(View view,
		...
		byte[] b = null;
		try {
			Bitmap bitmap = BitmapUtil.view2Bitmap(view);
			b = BitmapUtil.bitmap2Bytes(bitmap, compressFormat, true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return b;
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					LogUtil.d(clazzName, "Bitmap释放" + bitmap.toString());
					bitmap.recycle();
				}
			} catch (Exception e) {
			}
