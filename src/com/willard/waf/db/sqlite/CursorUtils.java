...
import com.willard.waf.db.table.Column;
...
public class CursorUtils {
    ...
    public static <T> T getEntity(final DbUtils db, final Cursor cursor, Class<T> entityType, long findCacheSequence) {
        ...
        try {
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                Column column = table.columnMap.get(columnName);//这里不会对注解为@Finder进行处理 只会对注解为@Column或注解为@Foreign进行处理
                if (column != null) {
            ...
            // init finder
            for (Finder finder : table.finderMap.values()) {//这里才处理注解为@Finder的属性
    ...
    private static class EntityTempCache {//实体临时缓存 key=value,实体名：实体
